...
class AdvancedTimesView extends StatefulWidget {
  const AdvancedTimesView({super.key});

  @override
  State<AdvancedTimesView> createState() => _AdvancedTimesViewState();
}
...
class _AdvancedTimesViewState extends State<AdvancedTimesView> {
  ...
  Widget build(BuildContext context) {
    ...
    Container GetDriverDataAdvanced(String name, String displayName)
    {
      ...
      try
      {
        for (int i = 0; i < mapData[name][0].length; i++)
        {
          if (mapData[name][0][i][2] != -1)
          {
            ...
            {
              double maxSpeed = -1;
              try{maxSpeed = mapData[name][1][i].reduce((value, element) => value > element ? value : element);}
              catch(e){}
              double minSpeed = -1;
              try{minSpeed = mapData[name][1][i].reduce((value, element) => value < element ? value : element);}
              catch(e){}
              double sector1 = -1;
              try{sector1 = mapData[name][0][i][7];}
              catch(e){}
              double sector2 = -1;
              try{sector2 = mapData[name][0][i][8];}
              catch(e){}
              double sector3 = -1;
              try{sector3 = mapData[name][0][i][9];}
              catch(e){}
              double res = -1;
              try{res = (mapData[name][4][i].map((element) => element > 95 ? 1 : 0).reduce((value, element) => value + element))/mapData[name][4][i].length * 100;}
              catch(e){}
              softs.add([i - cancelled, mapData[name][0][i][2], sector1, sector2, sector3, minSpeed, maxSpeed, mapData[name][0][i][19], mapData[name][0][i][0], mapData[name][15][i][5], mapData[name][15][i][1], mapData[name][15][i][7], res]); // weather is 15
            }
            else if (mapData[name][0][i][18] == 1)
            {
              ...
              mediums.add([i - cancelled, mapData[name][0][i][2], sector1, sector2, sector3, minSpeed, maxSpeed, mapData[name][0][i][19], mapData[name][0][i][0], mapData[name][15][i][5], mapData[name][15][i][1], mapData[name][15][i][7], res]); // weather is 15
            }
            else if (mapData[name][0][i][18] == 2)
            {
              ...
              hards.add([i - cancelled, mapData[name][0][i][2], sector1, sector2, sector3, minSpeed, maxSpeed, mapData[name][0][i][19], mapData[name][0][i][0], mapData[name][15][i][5], mapData[name][15][i][1], mapData[name][15][i][7], res]); // weather is 15
            }
            else if (mapData[name][0][i][18] == 3)
            {
              ...
              intermediates.add([i - cancelled, mapData[name][0][i][2], sector1, sector2, sector3, minSpeed, maxSpeed, mapData[name][0][i][19], mapData[name][0][i][0], mapData[name][15][i][5], mapData[name][15][i][1], mapData[name][15][i][7], res]); // weather is 15
            }
            else if (mapData[name][0][i][18] == 4)
            {
              ...
              wets.add([i - cancelled, mapData[name][0][i][2], sector1, sector2, sector3, minSpeed, maxSpeed, mapData[name][0][i][19], mapData[name][0][i][0], mapData[name][15][i][5], mapData[name][15][i][1], mapData[name][15][i][7], res]); // weather is 15
            }
          }
          else
          {
            ...
            List<TableRow> tableWidgets = [
              TableRow(
                children: [
                  ...
                  TableCell(
                    child: Center(child: Text('Sector 1')),
                  ),
                  TableCell(
                    child: Center(child: Text('Sector 2')),
                  ),
                  TableCell(
                    child: Center(child: Text('Sector 3')), // int maxNumber = numbers.reduce((value, element) => value > element ? value : element);
                  ),
                  TableCell(
                    child: Center(child: Text('Min Speed')),
                  ),
                  TableCell(
                    child: Center(child: Text('Max Speed')),
                  ),
                  TableCell(
              ...
              tableWidgets.add(
                TableRow(
                  children: [
                    ...
                    TableCell(
                      child: Center(child: Text(formatSeconds2(timesData[i][j][2]))),
                    ),
                    TableCell(
                      child: Center(child: Text(formatSeconds2(timesData[i][j][3]))),
                    ),
                    TableCell(
                      child: Center(child: Text(formatSeconds2(timesData[i][j][4]))),
                    ),
                    TableCell(
                    ...
                    TableCell(
                      child: Center(child: Text(timesData[i][j][7].toString())),
                    ),
                    TableCell(
                      child: Center(child: Text(formatSeconds(timesData[i][j][8]))),
                    ),
                    TableCell(
                      child: Center(child: Text(timesData[i][j][9].toString())),
                    ),
                    TableCell(
                      child: Center(child: Text(timesData[i][j][10].toString())),
                    ),
                    TableCell(
                      child: Center(child: Text(timesData[i][j][11].toString())),
                    ),
                    TableCell(
                      child: Center(child: Text(timesData[i][j][12].toStringAsFixed(1))),
                    ),
                  ]
                )
              );
    ...
    for (int i = 0; i < findMaxLength(drivers.length, drivers.length); i++)
    {
      containers.add(
        GetDriverDataAdvanced(drivers[i].toString(), "Driver Num: " + drivers[i].toString())
      );
    }
              ...
              for (int i = 0; i < screenshotControllers.length; i++)
              {
                screenshotControllers[i]
                  ...
                  try {
                    ...
                    final filePath = '${newFolder.path}/${drivers[i].toString()}.png';
