...
def replace_nan_with_negative_one(data):
    if isinstance(data, dict):
        for key, value in data.items():
            data[key] = replace_nan_with_negative_one(value)
    elif isinstance(data, list):
        for i, item in enumerate(data):
            data[i] = replace_nan_with_negative_one(item)
    elif isinstance(data, (float, np.float64)) and np.isnan(data):
        data = 99999
    return data

data = replace_nan_with_negative_one(data)
